...
//	private FRegionJpaService fRegionJpaService;
	 ...
	 */
	protected IkanJenis itemHeader = new IkanJenis();
...
//		remoteHostInfo= ((DashboardUI) getUI().getCurrent()).getRemoteHostInfo();
	...
	public void saveToDatabase() {
		...
		if (itemHeader.getID()==0) { //Berarti New
			...
			ikanJenisJpaService.createObject(itemHeader);
			...
			ikanJenisJpaService.updateObject(itemHeader);
	...
	public void deleteFromDatabase(IkanJenis domain_NullIfSingleDelete) {
		if (domain_NullIfSingleDelete==null) {
			ikanJenisJpaService.removeObject(itemHeader);
			...
			ikanJenisJpaService.removeObject(domain_NullIfSingleDelete);
	...
	public IkanJenis isKodeSudahAdaPerCompany(String kodeToCek, PemProv fcompanyBean) {
		/*
		 * NOTES: SubKelasIkan Sama semua
		 */
//		IkanJenis returnValue = new IkanJenis();
		IkanJenis returnValue = null;
		List<IkanJenis> listToFind = ikanJenisJpaService.findAllByField("kode1", kodeToCek.trim(), null);
		
		for (IkanJenis domain: listToFind) {
			try {
//				if (domain.getFdivisionBean().getFcompanyBean().equals(fcompanyBean) ) {
					returnValue = new IkanJenis();
					returnValue = domain;
					break;
//				}
			}catch (Exception e) {
	...
	public void reloadListHeader(){
		try {
			Set<IkanJenis> list = new HashSet<>();
...
//				list = new HashSet<IkanJenis>(ikanSubKelasJpaService.findAllByDivisionAndShareToCompany(userActive.getUnitKerjaBean().ge, false));
//				
//			}else if (userActive.getOrganizationLevel().equals(EnumOrganizationLevel.PEMDA)) {
//				list = new HashSet<IkanJenis>(ikanSubKelasJpaService.findAllByCompanyOnly(userActive.getUnitKerjaBean().getFcompanyBean().getID(), false));
//				
//			}else if (userActive.getOrganizationLevel().equals(EnumOrganizationLevel.SATKER)) {
//				list = new HashSet<IkanJenis>(ikanSubKelasJpaService.findAllByCompanyOnly(userActive.getUnitKerjaBean().getFcompanyBean().getID(), false));
			
//			} else {
				list = new HashSet<IkanJenis>(ikanJenisJpaService.findAll());
//			}
			for (IkanJenis domain: list) listHeader.put(domain.getID(), domain);
			
		}catch (Exception e) {
		}
		
	}
	public void reloadListHeaderWithCriteria(IkanJenis domainCriteria){
		reloadListHeader();
		
//		Set<IkanJenis> listTemp = new HashSet<IkanJenis>(listHeader);
//		listHeader = new HashSet<IkanJenis>();
//		for (IkanJenis domain: listTemp) {
