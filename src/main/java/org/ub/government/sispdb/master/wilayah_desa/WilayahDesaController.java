...
import org.ub.government.sispdb.master.wilayah_desa.WilayahDesaView.OnViewListener;
import org.ub.government.sispdb.model.Desa;
import org.ub.government.sispdb.model.Desa;
import org.ub.government.sispdb.model_enum.EnumStatusOperasiForm;

public class WilayahDesaController implements OnViewListener{
	protected WilayahDesaModel model;
	protected WilayahDesaView view;

	public WilayahDesaController(WilayahDesaView view) {
		model = new WilayahDesaModel();
		this.view = view;
	}
		...
		try {
			if(! (model.statusOperasiForm.equals(EnumStatusOperasiForm.ADD_NEW) || model.statusOperasiForm.equals(EnumStatusOperasiForm.EDIT_FORM)) ) {
		        try{
                    ...
                    Desa domainBean = new Desa();
                    domainBean = (Desa) model.tableModelHeader.get(selectedRow);
