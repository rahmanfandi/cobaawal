...
import org.ub.government.sispdb.master.pemerintahan_unitkerja.PemerintahanUnitKerjaView.OnViewListener;
import org.ub.government.sispdb.model.Kabupaten;
import org.ub.government.sispdb.model.Kabupaten;
import org.ub.government.sispdb.model_enum.EnumStatusOperasiForm;

public class PemerintahanUnitKerjaController implements OnViewListener{
	protected PemerintahanUnitKerjaModel model;
	protected PemerintahanUnitKerjaView view;

	public PemerintahanUnitKerjaController(PemerintahanUnitKerjaView view) {
		model = new PemerintahanUnitKerjaModel();
		this.view = view;
	}
...
//		view.setTableModelFilter();
	...
	public void aksiBtnDeleteForm() {
		if (model.statusOperasiForm.equals(EnumStatusOperasiForm.OPEN)) {
			 ...
			 */
			if (model.itemHeader.getKecamatanSet().size() >0) isValidDelete = false;
		...
		try {
			if(! (model.statusOperasiForm.equals(EnumStatusOperasiForm.ADD_NEW) || model.statusOperasiForm.equals(EnumStatusOperasiForm.EDIT_FORM)) ) {
		        try{
                    ...
                    Kabupaten domainBean = new Kabupaten();
                    domainBean = (Kabupaten) model.tableModelHeader.get(selectedRow);
