...
import org.ub.government.sispdb.master.pemerintahan_pemprop.PemerintahanPempropView.OnViewListener;
import org.ub.government.sispdb.model.Propinsi;
import org.ub.government.sispdb.model_enum.EnumStatusOperasiForm;

public class PemerintahanPempropController implements OnViewListener{
	protected PemerintahanPempropModel model;
	protected PemerintahanPempropView view;

	public PemerintahanPempropController(PemerintahanPempropView view) {
		model = new PemerintahanPempropModel();
		this.view = view;
	}
...
//		view.setTableModelFilter(0);
	...
	public void aksiBtnDeleteForm() {
		if (model.statusOperasiForm.equals(EnumStatusOperasiForm.OPEN)) {
			 ...
			 */
			if (model.itemHeader.getKabupatenSet().size() >0) isValidDelete = false;
			...
			if (event !=null) {
				if(! (model.statusOperasiForm.equals(EnumStatusOperasiForm.ADD_NEW) || model.statusOperasiForm.equals(EnumStatusOperasiForm.EDIT_FORM)) ) {
			        try{
	                    ...
	                    Propinsi domainBean = new Propinsi();
	                    domainBean = (Propinsi) model.tableModelHeader.get(selectedRow);
