...
import org.ub.government.sispdb.master.wilayah_kecamatan.WilayahKecamatanView.OnViewListener;
import org.ub.government.sispdb.model.Kecamatan;
import org.ub.government.sispdb.model.Kecamatan;
import org.ub.government.sispdb.model_enum.EnumStatusOperasiForm;

public class WilayahKecamatanController implements OnViewListener{
	protected WilayahKecamatanModel model;
	protected WilayahKecamatanView view;

	public WilayahKecamatanController(WilayahKecamatanView view) {
		model = new WilayahKecamatanModel();
		this.view = view;
	}
...
//		view.setTableModelFilter();
	...
	public void aksiBtnDeleteForm() {
		if (model.statusOperasiForm.equals(EnumStatusOperasiForm.OPEN)) {
			 ...
			 */
			if (model.itemHeader.getDesaSet().size() >0) isValidDelete = false;
		...
		try {
			if(! (model.statusOperasiForm.equals(EnumStatusOperasiForm.ADD_NEW) || model.statusOperasiForm.equals(EnumStatusOperasiForm.EDIT_FORM)) ) {
		        try{
                    ...
                    Kecamatan domainBean = new Kecamatan();
                    domainBean = (Kecamatan) model.tableModelHeader.get(selectedRow);
