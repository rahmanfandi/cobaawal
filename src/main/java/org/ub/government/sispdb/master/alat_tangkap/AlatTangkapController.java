...
import org.ub.government.sispdb.master.alat_tangkap.AlatTangkapView.OnViewListener;
import org.ub.government.sispdb.model.AlatTangkap;
import org.ub.government.sispdb.model_enum.EnumStatusOperasiForm;

public class AlatTangkapController implements OnViewListener{
	protected AlatTangkapModel model;
	protected AlatTangkapView view;

	public AlatTangkapController(AlatTangkapView view) {
		model = new AlatTangkapModel();
		this.view = view;
	}
			...
			if (event !=null) {
				if(! (model.statusOperasiForm.equals(EnumStatusOperasiForm.ADD_NEW) || model.statusOperasiForm.equals(EnumStatusOperasiForm.EDIT_FORM)) ) {
			        try{
	                    ...
	                    AlatTangkap domainBean = new AlatTangkap();
	                    domainBean = (AlatTangkap) model.tableModelHeader.get(selectedRow);
